...
**Functional over unit.** Prefer functional tests that exercise real `lat` commands against fixture directories over isolated unit tests. Unit tests are only for low-level edge cases that are hard to cover through fixtures (e.g. inline `parseSections` edge cases in `tests/omnidoc.test.ts`).

**Fixture-based.** Each test scenario is a static directory under `tests/cases/` with its own `omni.md/` and source files — a self-contained mini-project. No temp dirs or runtime file creation.
...
- [[check-md]] — Validating wiki links in omni.md markdown files
